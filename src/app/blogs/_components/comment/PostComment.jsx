...
function PostComment({ post: { comments, _id: postId } }) {
  ...
  return (
    <div className="mb-10">
      <Modal open={open} onClose={() => setOpen(false)} title={parent ? "پاسخ به نظز" : "نظر جدید"} description={parent ? "parent.user.name" : "نظر خود را وارد کنید"}>
                  ...
                  {comment.answers.map((item, index) => {
                    return (
                      <div key={item._id} className="relative">
                        <div
                          className={classNames("answer-item border border-secondary-100 bg-secondary-50/80 rounded-xl p-2 sm:p-4", {
